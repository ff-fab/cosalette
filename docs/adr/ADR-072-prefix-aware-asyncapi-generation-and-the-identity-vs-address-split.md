...
Accepted **Date:** 2026-09-08 | Amended **Date:** 2026-09-08
...
!!! note "Editorial note (2026-09-08)"
    Implementation clarification: `info.x-cosalette-topic-prefix` is emitted **conditionally**, not unconditionally as the Decision section's pseudocode comment suggests. The key is written only when the resolved prefix differs from `app.name`. Emitting it always would add a key to every existing document generated by an app that sets no `topic_prefix`, which contradicts this ADR's own guarantee that such documents stay byte-identical. Readers are unaffected: `SchemaRegistry.topic_prefix` holds the raw extension (`None` when absent) and `SchemaRegistry.resolved_topic_prefix` falls back to `info.title`, so consumers observe the same value either way.

!!! note "Editorial note (2026-09-08)"
    Implementation clarification: `SchemaRegistry.topic_prefix` deliberately stores the **raw** extension rather than eagerly resolving it to `info.title`. This keeps address parsing at the pre-ADR-072 depth-1 assumption for every legacy document, so a hand-written `info.title` containing `/` cannot silently corrupt derived device names. It is safe because `App` names cannot contain `/` (`_INVALID_MQTT_CHARS = frozenset("/+#\u0000")`), making the fallback exactly the old behaviour.

!!! note "Editorial note (2026-09-08)"
    Implementation scope note: the identity-vs-address split reaches two call sites this ADR's Decision section did not enumerate. First, `_resolve_device` (`_schema/_consumer_gen.py`) carried a **second** one-segment assumption in its malformed-address fallback, distinct from `_device_name_from_archetype`. Second, framework topics derived inside the consumer generator — `_availability_block` and the Home Assistant bridge entity's `state_topic` — were composed from the app identity rather than the transport prefix. Both now resolve through the prefix; the rule is unchanged, only its reach.

### Additional Negative Consequences

- Two caches are now keyed by the topic prefix — `App._asyncapi_cache` and the discovery-payload cache in `_wiring/_discovery.py` (keyed by `(config, topic_prefix)`). Any future code path that mutates registrations in place must invalidate the whole cache rather than a single slot, as `_resolve_app_settings` does after expanding name specs.
