...
- `exclude_none=True` on `validate_state_payload` (clause D) changes the device/stream wire payload for any `state_model` with optional fields currently published as explicit `null` — a key that was `null` becomes absent. This is contrary to the byte-stability expectation around ADR-045's amendment. The maintainer explicitly approved clause D on 2026-09-04, so this wire-payload change is an accepted consequence of the decision rather than an open question.
