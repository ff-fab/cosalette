...
!!! info "Under the hood"
    ...
    heartbeat fallback. `min_interval=` additionally bounds how often a trigger
    may start a run — see
    [Throttling a Trigger Storm](telemetry-advanced.md#throttling-a-trigger-storm).
    See [Triggerable Telemetry](telemetry-advanced.md#triggerable-telemetry).
...
app.add_telemetry(
    ...
    *,
    ...
    min_interval=None,       # storm throttle in seconds for triggered runs (requires triggerable=)
...
For coalescing groups, cron scheduling, retry/backoff resilience, and triggerable
on-demand reads, see [Advanced Telemetry Techniques](telemetry-advanced.md).

For lookup tables — interval guidelines, cron syntax, backoff strategy comparison,
and the timeout backstop semantics — see [Telemetry Reference](../reference/telemetry.md).
...
- [Advanced Telemetry Techniques](telemetry-advanced.md) — coalescing groups, cron
  scheduling, retry/backoff, and triggerable telemetry
