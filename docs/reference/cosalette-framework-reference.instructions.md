...
| Export              | Type       | Description                                        |
| ------------------- | ---------- | -------------------------------------------------- |
| `BackoffStrategy`   | Protocol   | `delay(attempt: int) -> float` — retry delay contract |
| `ExponentialBackoff` | class     | `min(base * 2^(attempt-1), max_delay)` with ±20% jitter |
| `LinearBackoff`     | class      | `min(step * attempt, max_delay)` with ±20% jitter  |
| `FixedBackoff`      | class      | Constant delay with ±20% jitter                    |
| `CircuitBreaker`    | class      | Consecutive-failure threshold → open/half-open/closed |
...
- **`interval`**: seconds between polls (> 0 for float). Can be a callable
...
- **`state_model`**: types the state channel payload in AsyncAPI schema generation. Resolution for schema generation: explicit `state_model` → return annotation → `{"type": "object"}`. **Runtime load-bearing on every publishing archetype** — one unconditional rule since 0.9.0 (ADR-068; `@app.device` / `@app.stream` since 0.6.0, ADR-045): declare it and published state is validated, raising `ReturnValidationError` on a mismatch. `@app.telemetry` / `@app.command` validate the handler *return value*; `@app.device` / `@app.stream` validate every `ctx.publish_state()` payload. The error is published to `{prefix}/{name}/error` and the state publish is suppressed. On `@app.telemetry` / `@app.command`, `state_model` **outranks the return annotation** (ADR-068 clause A); declaring both with different types warns at registration and `state_model` wins. A non-conforming plain `dict` return is rejected, not republished (clause B). Validated payloads dump with `exclude_none=True`, so an absent optional field is an omitted key rather than an explicit `null` (clauses C/D). `ctx.publish()` and `ctx.sub_entity(...)` channels are not covered. Omit it to publish unvalidated (breaking change: it used to be schema-only)
...
| `await .sleep_until(target, *, tz)`                  | Shutdown-aware sleep until wall-clock time     |
| `.on_command(handler)` or `@ctx.on_command("subtopic")` | Register command handler (with optional sub-topic routing) |
| `.commands(*, timeout=None)`                         | Async iterator yielding `Command` or `None` on timeout |
| `async with .sub_entity(name)`                       | Scoped sub-entity context manager              |
| `.command_handlers`                                  | Read-only mapping of registered command handlers |
...
| Method/Property                                      | Description                                    |
| ---------------------------------------------------- | ---------------------------------------------- |
| `await .publish_state(payload, *, retain=True)`      | Publish to `{prefix}/{device}/{name}/state`    |
| `.on_command(handler)`                               | Register command handler for this sub-entity's sub-topic |
| `.name`                                              | Sub-entity name                                |
| `.parent`                                            | Parent `DeviceContext`                         |
