...
VERSION_FEATURES: dict[str, list[str]] = {
    ...
    "0.9.0": [
        ...
        "virtual time. That makes 'no scheduled tick fired' and an exact "
        "publish count assertable, which FakeClock cannot express because "
        "its sleep() self-completes in one loop iteration. Additive: "
        "FakeClock is unchanged and stays the default; both doubles also "
        "gained a public advance(seconds) so tests stop writing _time "
