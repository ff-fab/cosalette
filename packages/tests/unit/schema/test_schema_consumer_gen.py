...
Test Techniques Used:
    ...
    - Error Guessing: DSL string break-out via quotes/backslashes in app,
      device, property names and channel addresses
"""
...
class TestOpenHabGenerator:
    ...
    def test_things_escapes_quote_in_channel_address(self) -> None:
        """A double quote in a channel address is escaped in stateTopic.

        Technique: Error Guessing — the address comes from the loaded document
        unvalidated, so a quoting metacharacter must not terminate the
        double-quoted ``stateTopic`` value early.
        """
        prop = _temp_property()
        channel = _temp_channel(
            address='my"app/sensor/state', properties={"temperature": prop}
        )
        registry = _make_registry({"q": channel})

        output = OpenHabGenerator(registry=registry).generate_things()

        assert 'stateTopic="my\\"app/sensor/state"' in output
        assert 'stateTopic="my"app' not in output
...
class TestOpenHabThingAvailability:
    ...
    def test_things_full_thing_block_matches_adr_079_layout(self) -> None:
        """A complete Thing renders exactly the ADR-079 "After" layout.

        Pins bracket placement between bridge ref and opening brace, 4-space
        parameter indentation, the comma on every parameter but the last, and
        the ``] {`` / ``Channels:`` line order.

        Technique: Specification-based Testing — golden .things layout.
        """
        prop = _temp_property()
        channel = _temp_channel(properties={"temperature": prop})
        registry = _make_registry({"temp": channel})
        ...
        assert things == (
            ...
            "\n"
            'Thing mqtt:topic:broker:myapp_sensor "myapp sensor" '
            "(mqtt:broker:broker) [\n"
            '    availabilityTopic="myapp/availability",\n'
            '    payloadAvailable="online",\n'
            '    payloadNotAvailable="offline"\n'
            "] {\n"
            "    Channels:\n"
            '        Type number : temperature "Temperature" [\n'
            '            stateTopic="myapp/sensor/state",\n'
            '            transformationPattern="JSONPATH:$.temperature"\n'
            "        ]\n"
            "}\n"
        )

    def test_multiple_named_devices_each_get_own_availability_topic(self) -> None:
        """Two named devices yield two Things, each pointing at its own topic.

        Technique: Specification-based Testing — per-Thing availability
        resolution is keyed on the resolved device, not the app.
        """
        desk = _temp_channel(
            address="myapp/desk/state", properties={"temperature": _temp_property()}
        )
        lamp = _temp_channel(
            address="myapp/lamp/state", properties={"temperature": _temp_property()}
        )
        registry = _make_registry(
            {"desk": desk, "lamp": lamp}, device_names=frozenset({"desk", "lamp"})
        )

        things = OpenHabGenerator(registry=registry).generate_things()

        assert 'availabilityTopic="myapp/desk/availability"' in things
        assert 'availabilityTopic="myapp/lamp/availability"' in things
        assert things.count("availabilityTopic=") == 2
...
class TestOpenHabThingParams:
    ...
    def test_thing_params_overrides_computed_availability(self) -> None:
        ...
        Technique: Specification-based Testing — author override beats the
        computed default.
        """
    ...
    def test_thing_params_merge_order_is_channel_address_then_property_name(
        self,
    ) -> None:
        """Across channels of one Thing, channel-address order is the primary key.

        The state channel (``wiz/desk/state``) sorts after the command channel
        (``wiz/desk/set``), so its property wins even though its name (``a``)
        sorts before the command property's (``z``) — property-name order
        alone would have picked the other value.

        Technique: Decision Table — channel-address vs property-name
        precedence.
        """
        state_prop = _temp_property(
            name="a", openhab=OpenHabOverrides(thing_params={"k": "from_state"})
        )
        cmd_prop = _temp_property(
            name="z",
            json_type="integer",
            openhab=OpenHabOverrides(thing_params={"k": "from_set"}),
        )
        state_ch = _temp_channel(
            address="wiz/desk/state", app_name="wiz", properties={"a": state_prop}
        )
        cmd_ch = _temp_channel(
            address="wiz/desk/set",
            app_name="wiz",
            archetype="command",
            direction="receive",
            properties={"z": cmd_prop},
        )
        registry = _make_registry({"s": state_ch, "c": cmd_ch})

        things = OpenHabGenerator(registry=registry).generate_things()

        assert 'k="from_state"' in things
        assert 'k="from_set"' not in things

    def test_thing_params_on_non_emittable_property_is_ignored(self) -> None:
        """A property that renders no channel contributes no thing_params.

        Mirrors channel_params: only emittable properties (consumer-annotated,
        single-valued) shape the Thing.

        Technique: Branch Coverage — emittable vs non-emittable contributor.
        """
        raw = PropertySchema(
            name="raw",
            json_schema={"type": "number"},
            consumer=None,
            openhab=OpenHabOverrides(thing_params={"leaked": "yes"}),
        )
        channel = _temp_channel(
            properties={"temperature": _temp_property(), "raw": raw}
        )
        registry = _make_registry({"temp": channel})

        things = OpenHabGenerator(registry=registry).generate_things()

        assert 'leaked="yes"' not in things
        assert 'availabilityTopic="myapp/availability"' in things
    ...
    @pytest.mark.parametrize(
        ...
        ids=["int", "float", "bool_true", "bool_false", "str_escaped"],
    )
    def test_thing_params_value_rendering_by_type(
        self, params: dict[str, object], expected: str
    ) -> None:
        """thing_params values render like channel_params: numbers bare,
        booleans quoted lowercase, strings escaped and quoted.

        Technique: Equivalence Partitioning — one case per
        ``_format_openhab_channel_param`` value class.
        """
        prop = _temp_property(openhab=OpenHabOverrides(thing_params=params))
        channel = _temp_channel(properties={"temperature": prop})
        registry = _make_registry({"temp": channel})

        things = OpenHabGenerator(registry=registry).generate_things()

        assert expected in things
...
class TestCompositeHaEntities:
    ...
    def test_json_attributes_topic_defaults_to_state_topic(self) -> None:
        ...
        Technique: Equivalence Partitioning — send-only channel with a template.
        """
        channel = _temp_channel(
    ...
    def test_json_attributes_topic_defaults_on_both_direction_composite(self) -> None:
        """direction='both' keeps a state topic, so the attributes topic defaults.

        Technique: Equivalence Partitioning over channel direction (the 'both'
        partition, alongside the send-only and receive-only cases).
        """
        channel = _temp_channel(
            address="myapp/panel/state",
            direction="both",
            ha_entities=(
                HaEntitySpec(
                    component="sensor",
                    name="panel",
                    extra={"json_attributes_template": "{{ {'x': value_json.x} }}"},
                ),
            ),
        )
        ...
        assert config["json_attributes_topic"] == "myapp/panel/state"

    def test_extra_state_topic_override_redirects_json_attributes_topic(self) -> None:
        """An extra-overridden state_topic redirects the defaulted attributes topic.

        Technique: Decision Table — (state_topic overridden × template present):
        the default must read the post-``extra`` state topic, not the channel
        address, so the two topics stay consistent (extra-wins-last invariant).
        """
        channel = _temp_channel(
            address="myapp/bulb/state",
            direction="send",
            ha_entities=(
                HaEntitySpec(
                    ...
                    name="Attrs",
                    extra={
                        "state_topic": "myapp/bulb/custom",
                        "json_attributes_template": "{{ {'x': value_json.x} }}",
                    },
                ),
            ),
        )
        ...
        assert config["state_topic"] == "myapp/bulb/custom"
        assert config["json_attributes_topic"] == "myapp/bulb/custom"
    ...
    def test_json_attributes_topic_absent_without_template(self) -> None:
        ...
        Technique: Decision Table — (template absent × state_topic present).
        """
        channel = _temp_channel(
            address="myapp/bulb/state",
            direction="send",
    ...
    def test_explicit_json_attributes_topic_is_preserved(self) -> None:
        ...
        Technique: Decision Table — (template present × explicit topic present).
        """
        channel = _temp_channel(
            address="myapp/bulb/state",
            direction="send",
    ...
    def test_command_only_composite_gets_no_json_attributes_topic(self) -> None:
        ...
        Technique: Boundary Value Analysis — the no-state-topic edge (template
        present but the entity publishes no state).
        """
        channel = _temp_channel(
